...
function alClic(e: MouseEvent) {
  if ((e.currentTarget as HTMLElement).getAttribute('href') === '#') e.preventDefault()
}

type Punto = [number, number]

// Tramo ondulado de a hacia b: la onda se anula en los extremos, así la línea toca cada objeto
function tramo(a: Punto, b: Punto, fase: number, amp: number, pts: Punto[]) {
...
onMounted(() => {
  ...
  const cj = caja.value!
  const svg = trazo.value!
  const hilos = [...svg.querySelectorAll<SVGPathElement>('.hilo')]
  const PROFUNDIDAD = [0.7, 1, 0.85]   // cuánto responde cada objeto al paralaje
  const objetos = [...cj.querySelectorAll<HTMLElement>('.objeto')]
    .map((el, i) => ({ el, i, dx: 0, dy: 0, prof: PROFUNDIDAD[i] || 1 }))

  const medir = () => svg.setAttribute('viewBox', `0 0 ${cj.clientWidth} ${cj.clientHeight}`)
  ...
  const alSalir = () => { raton.x = raton.y = 0; raton.px = raton.py = -1e4 }
  ...
  function paso(ahora = performance.now()) {
    ...
    const r = cj.getBoundingClientRect()
    ...
    const c: Punto[] = objetos.map(o => [o.el.offsetLeft + o.dx, o.el.offsetTop + o.dy])
    const n = c.length - 1
    const ini: Punto = [c[0]![0] - (c[1]![0] - c[0]![0]) * 0.9, c[0]![1] - (c[1]![1] - c[0]![1]) * 0.9]
    const fin: Punto = [c[n]![0] + (c[n]![0] - c[n - 1]![0]) * 0.9, c[n]![1] + (c[n]![1] - c[n - 1]![1]) * 0.9]
    const puntos: Punto[] = [ini, ...c, fin]
    const ola = reducido ? 0 : t * 0.6

    hilos.forEach((h, j) => {
      const pts: Punto[] = [puntos[0]!]
