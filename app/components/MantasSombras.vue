<template>
  <canvas id="sombras" ref="lienzo" aria-hidden="true"></canvas>
</template>

<script setup lang="ts">
/* REEF Records · Sombras de mantarrayas (canvas 2D)
   Se queda en canvas 2D: son trazos Bézier con ctx.filter = blur(); three.js no aporta nada y
   rehacer el desenfoque en GLSL cambiaría el aspecto. */
...
onMounted(() => {
  ...
  function ajustarSombras() {
    ...
    cv.width = Math.round(SW * RES)
    cv.height = Math.round(SH * RES)
  }
  ajustarSombras()
  ...
  function crearManta(tam: number, prof: number, x: number, y: number) {
    return {
      tam, prof, x, y,
      ...
      t: Math.random() * 10,
      destino: null as null | { x: number, y: number }
    }
  }
  const mantas = [
  ...
  function nuevoDestino(m: ReturnType<typeof crearManta>) {
  ...
  const K = 100 / 197
  const aLocal = ([rx, ry]: P): P => [-(ry - 118) * K, (rx - 207) * K]
  const derecha = MITAD_DERECHA.map(s => s.map(aLocal) as Tramo)
  const izquierda = derecha.slice().reverse().map(([p0, c1, c2, p3]) =>
    [[p3[0], -p3[1]], [c2[0], -c2[1]], [c1[0], -c1[1]], [p0[0], -p0[1]]] as Tramo)
  const CONTORNO = derecha.concat(izquierda)
  const COLA = [aLocal([207, 232]), aLocal([209, 254]), aLocal([190, 262]), aLocal([146, 248])]

  function silueta(c: CanvasRenderingContext2D, _s: number, apertura: number, punta: number) {
    // cada punto se pliega según su distancia al cuerpo: la punta del ala va con retraso
    const pliega = ([x, y]: P): P => {
    ...
    const [p0, q1, q2, p3] = COLA as [P, P, P, P]
    c.beginPath()
  ...
  function nadar(ahora: number) {
    const dt = Math.min((ahora - antes) / 1000, 0.05)
    antes = ahora
    const visibles = Math.max(0, 1 - reef.prof * 1.4)
    cv.style.opacity = String(visibles)
    if (visibles <= 0) { raf = requestAnimationFrame(nadar); return }
    ...
    mantas.forEach(m => {
      ...
      const ala = (f: number) => 0.74 + 0.26 * suave(Math.abs(Math.cos(f)))
    ...
    raf = requestAnimationFrame(nadar)
  }
  nadar(antes)

  limpiar = () => {
    cancelAnimationFrame(raf)
    window.removeEventListener('resize', ajustarSombras)
  }
})
