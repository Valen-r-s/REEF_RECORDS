...
(() => {
  ...
  const datos = [];   // base x,y | normal x,y | recorrido, semilla, profundidad, tamaño | inicio x,y | tipo
  const azar = (a, b) => a + Math.random() * (b - a);
  const inicioAleatorio = () => [azar(-1.9, 1.9), azar(-1.15, 1.15)];
  ...
  const atributo = (nombre, tam, desp) => {
    ...
    gl.vertexAttribPointer(loc, tam, gl.FLOAT, false, STRIDE * F, desp * F);
  };
  atributo('aBase', 2, 0);
  atributo('aNormal', 2, 2);
  atributo('aDato', 4, 4);
  atributo('aInicio', 2, 8);
  atributo('aTipo', 1, 10);

  const U = {};
  ['uT', 'uForma', 'uAsp', 'uEscala', 'uDpr', 'uGiro', 'uCentro', 'uPos', 'uMouse', 'uOpac']
    .forEach(n => U[n] = gl.getUniformLocation(prog, n));

  gl.enable(gl.BLEND);
  gl.blendFunc(gl.ONE, gl.ONE);   // suma de luz: las hebras brillan donde se cruzan
  ...
  (function dibujar() {
    ...
    const pos = movil ? [0.0, 0.68] : [asp * 0.43, 0.08];
    ...
    gl.uniform1f(U.uT, t);
    gl.uniform1f(U.uForma, window.REEF.forma);
    gl.uniform1f(U.uAsp, asp);
    gl.uniform1f(U.uEscala, ancho / 380);
    gl.uniform1f(U.uDpr, dpr);
    gl.uniform1f(U.uGiro, 0.1 + Math.sin(t * 0.2) * 0.03);
    gl.uniform2f(U.uCentro, 205, 70);
